...
export function Contact() {
  return (
    ...
    >
      <div
        aria-hidden
        className="pointer-events-none absolute -right-32 top-0 h-[30rem] w-[30rem] rounded-full bg-brand/10 blur-[120px]"
      />
      <Container className="relative">
        <div className="grid gap-12 lg:grid-cols-[0.85fr_1.15fr] lg:items-start">
          ...
          <div className="text-ink-900">
            ...
            <p className="mt-4 max-w-md leading-relaxed text-sand-700">
...
function ContactRail({
  ...
  const inner = (
    <>
      <span className="grid h-11 w-11 shrink-0 place-items-center rounded-lg bg-brand-soft text-brand ring-1 ring-brand/10">
        <Icon className="h-5 w-5" />
      </span>
      <span>
        <span className="block text-xs uppercase tracking-wider text-sand-700">
          {label}
        </span>
